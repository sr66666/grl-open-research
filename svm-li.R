...
svm_fit <- train(Li ~ ., data = train, method = "svmRadial",
...
svm_model <- svm(Li ~ ., data = train, kernel = "radial", cost = best_C, gamma = best_sigma)
...
write.csv(as.matrix(statistics_by_class), file = "D:/data/svm_li_xingneng.csv")
...
write.csv(new_predictions, file = "D:/data/gbrlirfpre_svm.csv")